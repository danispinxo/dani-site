...
const IncompleteList = ({ incompleteTasks, handleMarkAsDone, handleEditTask, fetchTasks, createNewList }) => {
  ...
  const handleDeleteSelectedTasks = async (ids) => {
    try {
      const { error } = await supabase.from('todo_list_item').delete().in('id', ids);
      if (!error) fetchTasks();
    } catch (error) {
      ...
      <div>
        {selectedIds.length > 0 && (
          <div className="selected-task-buttons">
            <button onClick={() => handleDeleteSelectedTasks(selectedIds)}>Delete Selected</button>
            <button onClick={() => handleTransferToNewList(selectedIds)}>Transfer to New List</button>
          </div>
        )}
      </div>
      <Row>
            ...
            <Col key={`category-${index}`}>
              ...
              <ol className="todo-list">
                {categorizedTasks[category].map((t, index) => (
                  <li key={index} className={`todo-list-item ${selectedIds.includes(t.id) ? 'selected' : ''}`}>
                    <>
                      <span onClick={handleSelectListItem(t)}>{shortenText(t.text)}</span>
